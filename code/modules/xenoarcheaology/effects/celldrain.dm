...
/datum/artifact_effect/celldrain/DoEffectAura()
	if(holder)
		...
		for (var/obj/machinery/power/apc/C in physical_range(200, T))
			for (var/obj/item/weapon/cell/B in C.contents)
				B.charge = max(B.charge - 50,0)
		for (var/obj/machinery/power/smes/S in range (src.effectrange,src))
			S.charge = max(S.charge - 100,0)
		for (var/mob/living/silicon/robot/M in physical_range(50, T))
...
/datum/artifact_effect/celldrain/DoEffectPulse()
	if(holder)
		...
		for (var/mob/living/silicon/robot/M in physical_range(100, T))
			for (var/obj/item/weapon/cell/D in M.contents)
